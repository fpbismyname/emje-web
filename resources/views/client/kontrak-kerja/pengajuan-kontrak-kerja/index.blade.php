<x-layouts.client-app title="Daftar pengajuan kontrak kerja">
    {{-- Header --}}
    <div class="flex flex-row justify-between gap-4 flex-wrap">
        ...
        <form method="get" class="flex flex-row md:flex-1 gap-4 flex-wrap">
            ...
            <input type="text" class="input" name="search" value="{{ request('search') }}"
                placeholder="Cari pengajuan kontrak" />
                    ...
                    <li class="list-row">
                        ...
                        <div class="list-col-grow">
                            <div class="flex flex-col gap-2">
                                {{ $item->kontrak_kerja->nama_perusahaan }}
                                <div class="flex flex-row gap-4">
                                    <div class="badge badge-sm badge-primary">
                                        {{ $item->kontrak_kerja->kategori_kontrak_kerja->label() }}
                                    </div>
                                ...
                                <div class="flex flex-col">
                                    <small>
                                        Rentang gaji : {{ $item->kontrak_kerja->formatted_gaji_terendah }} -
                                        {{ $item->kontrak_kerja->formatted_gaji_tertinggi }}
                                    </small>
                                    <small>
                                        Durasi kontrak : {{ $item->kontrak_kerja->formatted_durasi_kontrak_kerja }}
                                    </small>
                                </div>
                            </div>
                        </div>
                        <div>
                            <div class="flex flex-row gap-2">
                                <a href="{{ route('client.kontrak-kerja.pengajuan-kontrak-kerja.show', [$item->id]) }}"
