<x-layouts.client-app title="Detail akun pengguna">
    <form action="{{ route('client.pengaturan.update.pengguna') }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('put')
        {{-- Data akun --}}
        ...
        <div class="grid md:grid-cols-2 gap-4 rounded-box p-4">
            ...
            <fieldset class="fieldset">
                ...
                @if ($user->profil_user?->foto_profil)
                    <a target="_blank"
                        href="{{ route('storage.private.show', ['file' => $user->profil_user?->foto_profil]) }}"
                        class="text-sm link link-hover link-primary">Lihat
                        selengkapnya</a>
                @else
                    <p class="text-sm">Tidak ada foto profil</p>
                @endif
            ...
            <fieldset class="fieldset">
                ...
                @if ($user->profil_user?->ktp)
                    <a target="_blank" href="{{ route('storage.private.show', ['file' => $user->profil_user?->ktp]) }}"
                        class="text-sm link link-hover link-primary">Lihat
                        selengkapnya</a>
                @else
            ...
            <fieldset class="fieldset">
                ...
                @if ($user->profil_user?->ijazah)
                    <a target="_blank"
                        href="{{ route('storage.private.show', ['file' => $user->profil_user?->ijazah]) }}"
                        class="text-sm link link-hover link-primary">Lihat
                        selengkapnya</a>
                @else
                    <p class="text-sm">Tidak ada ijazah</p>
                @endif
