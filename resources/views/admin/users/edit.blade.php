        ...
        <div class="grid md:grid-cols-2 gap-4 rounded-box p-4">
            ...
            <fieldset class="fieldset">
                ...
                <select name="role" id="role" class="select validator w-full" required>
                    ...
                    @if (in_array($user->role, App\Enums\User\RoleEnum::admin_user()))
                        @foreach (App\Enums\User\RoleEnum::admin_user() as $role)
                            <option value="{{ $role->value }}" @selected($user->role == $role)>{{ $role->label() }}
                            </option>
                        @endforeach
                    @endif
                    @if (in_array($user->role, App\Enums\User\RoleEnum::client_user()))
