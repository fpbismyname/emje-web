<x-layouts.admin-app title="Tambah pelatihan">
    <form action="{{ route('admin.pelatihan.store') }}" method="POST" class="space-y-4">
        @csrf
        @method('post')
        ...
        <div class="grid md:grid-cols-2 gap-4 rounded-box p-4">
            ...
            {{-- Nominal Biaya --}}
            <fieldset class="fieldset">
                <legend class="fieldset-legend">Nominal Biaya <small id="preview-nominal-biaya"></small></legend>
                <input type="number" name="nominal_biaya" required
                    oninput="window.render_currency_to(this,'preview-nominal-biaya')" value="{{ old('nominal_biaya') }}"
                    min="0" class="input validator w-full">
                ...
                <select name="kategori_pelatihan" required class="select validator w-full">
                    ...
                    @foreach (App\Enums\Pelatihan\KategoriPelatihanEnum::cases() as $kategori)
                        <option value="{{ $kategori->value }}"
                            {{ old('kategori_pelatihan') == $kategori->value ? 'selected' : '' }}>
                            {{ $kategori->label() }}
                        </option>
                    @endforeach
                </select>
            ...
            <fieldset class="fieldset">
                ...
                <textarea name="deskripsi" required class="textarea validator w-full" rows="4" style="white-space: pre-wrap">{{ old('deskripsi') }}</textarea>
