<x-layouts.admin-app
    ...
    {{-- Data pelatihan --}}
    <form
        action="{{ route('admin.pelatihan-peserta.detail.sertifikasi.update', [$profil_user->id, $pelatihan_peserta->id, $sertifikat->id]) }}"
        ...
        <div class="grid md:grid-cols-2 gap-4 rounded-box p-4">
            ...
            <fieldset class="fieldset">
                ...
                <select name="jenis_sertifikat" class="select w-full validator">
                    ...
                    @foreach (App\Enums\Pelatihan\JenisSertifikatEnum::cases() as $jenis)
                        <option value="{{ $jenis->value }}" @selected(old('jenis_sertifikat', $sertifikat->jenis_sertifikat->value) == $jenis->value)>
                            {{ $jenis->label() }}
                        </option>
                    @endforeach
                    </option>
            ...
            {{-- Sertifikat --}}
            <fieldset class="fieldset">
                <legend class="fieldset-legend">Sertifikat (Upload untuk mengganti sertifikat sebelumnya)</legend>
