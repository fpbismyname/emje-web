<?php

return [
    'title' => 'PT Emje Pandanjaya Indonesia', // No change in this line
    'tagline' => 'Mendorong Pertumbuhan Ekonomi Melalui Layanan Terpercaya dan Berkelanjutan',
    'description' => 'PT Emje Pandanjaya Indonesia adalah perusahaan berbasis di Kabupaten Cianjur, Jawa Barat, yang bergerak di bidang pelatihan kerja, penempatan tenaga kerja, serta perdagangan besar berbagai komoditas dan produk agroindustri. Terdaftar resmi dalam sistem OSS sebagai perusahaan PMDN skala usaha kecil, kami berkomitmen mendukung pengembangan sumber daya manusia dan menyediakan layanan perdagangan serta jasa profesional bagi berbagai mitra bisnis. Dengan cakupan usaha yang fleksibel dan terintegrasi, PT Emje Pandanjaya Indonesia hadir untuk mendorong pertumbuhan ekonomi lokal dan nasional melalui layanan yang terpercaya dan berkelanjutan.',
    'about' => [
        'paragraph_1' => 'PT Emje Pandanjaya Indonesia adalah perusahaan yang berdedikasi untuk memajukan potensi sumber daya manusia dan ekonomi lokal. Berlokasi di Kabupaten Cianjur, Jawa Barat, kami memiliki fokus utama pada pelatihan kerja dan penempatan tenaga kerja, memastikan individu memiliki keterampilan yang relevan dan kesempatan karir yang menjanjikan.',
        'paragraph_2' => 'Selain itu, kami juga aktif dalam perdagangan besar berbagai komoditas dan produk agroindustri, mendukung rantai pasok lokal dan nasional. Sebagai perusahaan PMDN skala kecil yang terdaftar resmi di OSS, kami menjunjung tinggi integritas dan profesionalisme dalam setiap aspek bisnis kami, berupaya menjadi mitra terpercaya bagi pertumbuhan berkelanjutan.'
    ],
    'icon' => 'icon/company_icon.png',
    'services' => [
        [
            'title' => 'Pelatihan Kerja',
            'description' => 'Menyediakan program pelatihan komprehensif untuk meningkatkan keterampilan dan daya saing tenaga kerja.'
        ],
        [
            'title' => 'Penempatan Tenaga Kerja',
            'description' => 'Menghubungkan talenta terbaik dengan peluang kerja yang sesuai di berbagai sektor industri.'
        ],
        [
            'title' => 'Perdagangan Komoditas',
            'description' => 'Distribusi berbagai komoditas penting untuk mendukung kebutuhan industri dan masyarakat.'
        ],
        [
            'title' => 'Produk Agroindustri',
            'description' => 'Menyediakan produk-produk berkualitas dari sektor pertanian dan industri pengolahan.'
        ],
        [
            'title' => 'Jasa Konsultasi Bisnis',
            'description' => 'Memberikan solusi dan saran ahli untuk pengembangan dan optimalisasi bisnis Anda.'
        ],
        [
            'title' => 'Pengembangan SDM',
            'description' => 'Program terstruktur untuk meningkatkan kapasitas dan kapabilitas sumber daya manusia.'
        ]
    ],
    'contact' => [
        'email' => 'info@emjepandanjaya.id',
        'phone' => '+6283181686431',
        'address' => 'Jl. Raya Cianjur No. 123, Cianjur, Jawa Barat',
        'whatsapp' => 'https://wa.me/+6283181686431'
    ],
    'prosedur_pengajuan_surat_kontrak' => <<<TEXT
    Prosedur pengajuan kontrak kerja
    - Download surat kontrak kerja.
    - Pastikan kualifikasi pelatihan anda sesuai dengan kategori kontrak kerja.
    - Edit surat pengajuan kontrak kerja, Isi nama dan tanda tangan anda.
    - Kirim surat pengajuan kontrak kerja pada form dibawah.
    TEXT
];
